...
- **Agent-to-agent messaging** — opt-in `--a2a` runs a companion A2A adapter container, exposing a standard AgentCard plus JSON-RPC `message/send`. Stock instances are unaffected.
...
Make instances talk to each other with A2A (opt-in):
