...
当前 `ClawCU` 命令界面参考。覆盖 OpenClaw 与 Hermes 共享生命周期命令、provider 管理、孤儿实例恢复、A2A companion adapter，以及运行时默认值。
...
## 11. Agent-to-Agent 消息

所有 A2A 行为通过创建时的 `--a2a` 开启；不加则已有实例一切不变。
