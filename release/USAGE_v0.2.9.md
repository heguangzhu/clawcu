...
🌐 Language:
[English](USAGE_v0.2.9.md) | [中文](USAGE_v0.2.9.zh-CN.md)

Release Scope: `v0.2.9`

Command reference for `ClawCU v0.2.9`. Covers the shared command surface, OpenClaw / Hermes service differences, the orphan-instance lifecycle (introduced in `v0.2.6`), the `v0.2.9` UX additions (`clawcu list` available-versions footer, bare-invoke help), and the operational defaults.
...
- `--remote` / `--no-remote` — toggle the "Available versions" footer registry fetch (default on). Footer shows the top 10 stable releases per service (OpenClaw, Hermes), newest first; prereleases (`-beta`, `-rc`, `-alpha`) are filtered. Omitted in `--json` / `--agents` / `--removed` views
...
- This usage guide describes the command surface for `v0.2.9`.
- For release context, see [RELEASE_v0.2.9.md](RELEASE_v0.2.9.md).
