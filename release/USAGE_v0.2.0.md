...
| `clawcu clone <source> --name <name> [--datadir <path>] [--port <port>] [--version <v>] [--include-secrets/--exclude-secrets]` | Copy a source instance into a new isolated experiment instance. The source's data directory is always copied. By default the source's env file (API keys / tokens / provider secrets) is ALSO copied — pass `--exclude-secrets` to start with an empty env; ClawCU will print a reminder to re-seed credentials via `clawcu setenv` before use. Pass `--version <v>` to switch the clone to a different service version at copy time, which enables a safe "clone then upgrade" workflow without touching the original (the clone's history records both the source version and the target version for provenance). |
