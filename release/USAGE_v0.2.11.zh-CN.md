...
`ClawCU v0.2.11` 的命令参考。覆盖 OpenClaw 与 Hermes 共享的命令界面、两者差异、在 `v0.2.6` 引入的孤儿实例生命周期、`v0.2.9` 的 UX 增补（`clawcu list` 可用版本页脚、空参数 help）、`v0.2.10` 的打磨（页脚按天缓存、registry 不通时回退到本地镜像）、以及 `v0.2.11` 的 runtime image 覆盖流程（`create` / `upgrade --image`，并把镜像链持久化到 `recreate` / 孤儿恢复 / `rollback`），以及运行时默认值。
...
- `--image <ref>` —— 可选 runtime image 覆盖。设置后 Docker 启动该镜像，但实例记录里的 OpenClaw 版本仍取 `--version`
...
- `--image <ref>` —— 可选 runtime image 覆盖。设置后 Docker 启动该镜像，但实例记录里的 Hermes 版本仍取 `--version`
- `datadir` 默认 `~/.clawcu/<name>`
...
- 对已受管实例，`recreate` 始终复用保存下来的 runtime image，不负责切换 artifact
...
- `--version <v>` —— 除 `--list-versions` 外必填。即使同时传了 `--image`，实例记录里的逻辑版本仍取这里
- `--image <ref>` —— 可选 runtime image 覆盖。设置后 Docker 启动该镜像，而不是按 `--version` 去配置的 image repo 里推导默认 tag
...
- 选中的 runtime image 会持久化，因此后续 `recreate`、孤儿恢复和 `rollback` 会沿用同一条镜像链
...
- 回滚不仅恢复 snapshot 数据，也会恢复该历史转换记录下来的 runtime image
...
- 本文档描述 `v0.2.11` 命令界面。
- 发布上下文见 [RELEASE_v0.2.11.zh-CN.md](RELEASE_v0.2.11.zh-CN.md)。
