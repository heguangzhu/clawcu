# ClawCU Usage v0.2.11

🌐 Language:
[English](USAGE_v0.2.11.md) | [中文](USAGE_v0.2.11.zh-CN.md)

Release Scope: `v0.2.11`

Command reference for `ClawCU v0.2.11`. Covers the shared command surface, OpenClaw / Hermes service differences, the orphan-instance lifecycle (introduced in `v0.2.6`), the `v0.2.9` UX additions (`clawcu list` available-versions footer, bare-invoke help), the `v0.2.10` polish (daily cache for the footer, local-image fallback when the registry is unreachable), the `v0.2.11` runtime-image override flow (`create` / `upgrade --image`, persisted image chain across `recreate` / orphan recovery / `rollback`), and the operational defaults.
...
- `--image <ref>` — optional runtime image override. When set, Docker starts this image while `--version` remains the recorded OpenClaw version
...
- `--image <ref>` — optional runtime image override. When set, Docker starts this image while `--version` remains the recorded Hermes version
- `datadir` defaults to `~/.clawcu/<name>`
...
- Existing managed instances always reuse their saved runtime image; `recreate` does not switch artifacts
...
- `--version <v>` — required target version unless `--list-versions` is passed. Remains the logical version recorded on the instance even when `--image` is used
- `--image <ref>` — optional runtime image override. When set, Docker starts this image instead of the configured repo's default tag for `--version`
...
- Rollback restores both the snapshot data and the runtime image recorded for the selected historical transition
...
- This usage guide describes the command surface for `v0.2.11`.
- For release context, see [RELEASE_v0.2.11.md](RELEASE_v0.2.11.md).
