...
`ClawCU v0.2.10` 的命令参考。覆盖 OpenClaw 与 Hermes 共享的命令界面、两者差异、在 `v0.2.6` 引入的孤儿实例生命周期、`v0.2.9` 的 UX 增补（`clawcu list` 可用版本页脚、空参数 help）、`v0.2.10` 的打磨（页脚按天缓存、registry 不通时回退到本地镜像），以及运行时默认值。
...
- 本文档描述 `v0.2.10` 命令界面。
- 发布上下文见 [RELEASE_v0.2.10.zh-CN.md](RELEASE_v0.2.10.zh-CN.md)。
