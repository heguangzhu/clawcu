...
| `clawcu restart <name> [--recreate-if-config-changed]` | Restart a managed instance. With `--recreate-if-config-changed`, ClawCU inspects the live container and, if env drift is detected (e.g. after `setenv` without `--apply`) or the container is missing, promotes the restart to a full `recreate` so the new env file takes effect. Without the flag it is a plain `docker restart`. |
