...
| `clawcu upgrade <name> [--version <v>] [--list-versions] [--dry-run] [--yes] [--json]` | Upgrade an instance to a new service version or tag. ClawCU snapshots the instance home plus the matching env path before replacing the container, and the env file is preserved across the upgrade. `--list-versions` shows candidate versions (local Docker images for the service's image repo + this instance's version history) without requiring `--version`. `--dry-run` prints the upgrade plan (current → target, datadir, env carry-over summary, projected image tag, snapshot path) without touching Docker or disk. The normal path renders the plan first and then asks for confirmation — pass `--yes` / `-y` to skip the prompt (required in non-interactive shells). `--json` emits the plan / version list as JSON. |
