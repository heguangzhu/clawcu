...
标准 A2A 任务生命周期方法通过 `a2a-sdk` 的 `InMemoryTaskStore` 支持。
...
Adapter 也在 `POST /mcp` 上通过 JSON-RPC 提供 MCP，暴露两个工具：

- `a2a_call_peer(to, message, registry_url?, timeout_seconds?)`
- `a2a_list_peers(registry_url?, timeout_seconds?)`

`a2a_call_peer` 会在 A2A registry 里查找 `to`，向对端发送标准 A2A `message/send` 请求，并返回文本内容与结构化 task 数据。`a2a_list_peers` 列出 registry 里的名称、角色、技能与端点，让本地 agent 能发现对端实际可能叫 `a2a-smoke-analyst`，而不是 `analyst`。
...
**开销多大？**
一个轻量 Python 容器，使用 `a2a-sdk`。内存：闲时 <50 MB。CPU：静息 0；每次请求的成本主要来自下游 LLM 调用。
