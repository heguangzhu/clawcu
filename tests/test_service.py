...
def test_set_clawcu_home_does_not_persist_bootstrap_home_when_switch_fails(
    temp_clawcu_home, tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("HOME", str(tmp_path / "user-home"))
    service, _, openclaw, store = make_service(temp_clawcu_home)
    original_home = store.paths.home

    def fail_switch(_home: str) -> None:
        raise PermissionError("no write access")

    monkeypatch.setattr(store, "switch_home", fail_switch)

    with pytest.raises(PermissionError, match="no write access"):
        service.set_clawcu_home(str(tmp_path / "blocked-home"))

    assert store.paths.home == original_home
    assert store.get_bootstrap_home() is None
    assert not hasattr(openclaw, "store")
    assert not bootstrap_config_path().exists()
