...
| `clawcu upgrade <name> [--version <v>] [--list-versions] [--remote/--no-remote] [--dry-run] [--yes] [--json]` | 将实例升级到新的服务版本或 tag。升级前会自动快照实例 home 和对应的 env 路径，env 文件会在升级过程中保留。`--list-versions` 不需要 `--version`，会列出候选版本：始终包含该实例的版本历史和本地已拉取的镜像；加上 `--remote`（默认开启）还会通过 Docker Registry v2 API 向配置好的 image 仓库（GHCR / Docker Hub）查询 release tag。远端查询是 best-effort——网络或鉴权失败时会回退到本地视图并打印警告行，不会让命令挂掉。`--no-remote` 可以完全跳过远端查询（CI / 离线环境下有用）。`--dry-run` 会打印升级计划（当前版本 → 目标版本、datadir、env 保留摘要、投射的 image tag、快照路径），不动 Docker 或磁盘。正常路径会先渲染计划然后要求二次确认——加 `--yes` / `-y` 跳过提示（非交互式 shell 里必须加）。`--json` 会把 plan / 版本列表以 JSON 形式输出。 |
