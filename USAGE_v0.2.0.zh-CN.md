...
| `clawcu upgrade <name> [--version <v>] [--list-versions] [--remote/--no-remote] [--all-versions] [--dry-run] [--yes] [--json]` | 将实例升级到新的服务版本或 tag。升级前会自动快照实例 home 和对应的 env 路径，env 文件会在升级过程中保留。`--list-versions` 不需要 `--version`，会列出候选版本：始终包含该实例的版本历史和本地已拉取的镜像；加上 `--remote`（默认开启）还会通过 Docker Registry v2 API 向配置好的 image 仓库（GHCR / Docker Hub）查询 release tag。远端查询是 best-effort——网络或鉴权失败时会回退到本地视图并打印警告行，不会让命令挂掉。`--no-remote` 可以完全跳过远端查询（CI / 离线环境下有用）。默认只显示远端最近的 10 个 release tag 以保持输出可读，加 `--all-versions` 可显示全部远端列表；`--json` 不受影响，始终返回完整数据，方便程序化消费。`--dry-run` 会打印升级计划（当前版本 → 目标版本、datadir、env 保留摘要、投射的 image tag、快照路径），不动 Docker 或磁盘。正常路径会先渲染计划然后要求二次确认——加 `--yes` / `-y` 跳过提示（非交互式 shell 里必须加）。`--json` 会把 plan / 版本列表以 JSON 形式输出。 |
| `clawcu rollback <name> [--to <version>] [--list] [--dry-run] [--yes] [--json]` | 把实例回退到某个已保存的快照。不加 `--to` 时默认回退到"上一次可逆切换"。`--to <version>` 会在历史里反向查找 "restores to" 匹配该版本的最近一条事件。`--list` 列出该实例所有可用快照目标（action、restore 版本、快照路径、快照是否仍在磁盘上），不会动 Docker。`--dry-run` 会渲染回滚计划（当前版本 → 目标版本、datadir、env 恢复信息、来源事件、投射 image、安全快照路径），不动 Docker 或磁盘。正常路径会先渲染计划再要求二次确认，加 `--yes` / `-y` 跳过提示（非交互式 shell 里必须加）。`--json` 会把 plan / target 列表以 JSON 形式输出。 |
| `clawcu clone <source> --name <name> [--datadir <path>] [--port <port>] [--version <v>] [--include-secrets/--exclude-secrets]` | 复制源实例，生成新的隔离实验实例。数据目录始终复制。默认会连同源实例的 env 文件（API key / token / provider 凭证）一起复制；加 `--exclude-secrets` 则跳过 env 复制，克隆以空 env 启动，ClawCU 会打印提示让你通过 `clawcu setenv` 重新注入凭证。加 `--version <v>` 可以在克隆的同时切换到另一个服务版本，实现"先 clone 再 upgrade"的安全升级预演——源实例不会被动，克隆实例的 history 会同时记录源版本和目标版本以便溯源。 |
